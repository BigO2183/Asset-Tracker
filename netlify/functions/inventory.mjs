...
export default async (request) => {
  try {
    ...
    const recordKey = `workspace:${session.workspaceId}:inventory-state`;
    ...
    console.error('SimpleStock workspace inventory error:', error);
    return json(
      { ok: false, error: error?.message || 'Cloud storage error.' },
      500
    );
  }
};
